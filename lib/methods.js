      ...
      Meteor.call("addShoppingTalk",shopping_obj,item, {returnStubValue: true},
        function(error,result){
          ...
          if(r.result.action == "shoppingList.add"){
            ...
            if(r.result.parameters.page == "/"){
              Router.go('/');
              var t = "Going to home page";
              var pierreWords = new SpeechSynthesisUtterance(t);
              window.speechSynthesis.speak(pierreWords);
            } else {
              Router.go('/'+t);
            ...
            if(r.result.parameters.diet != ""){
              var diet = r.result.parameters.Diet;
            } else {
              diet = null;
            }
            Session.set("search",{
            ...
            var pierreWords = new SpeechSynthesisUtterance(t);
            window.speechSynthesis.speak(pierreWords);
          } else if(r.result.action == "fridge.remove"){
            var s = r.result.parameters.fridgeItem;
            const fridgeItem = Fridge.findOne({text:r.result.parameters.fridgeItem});
            ...
            var pierreWords = new SpeechSynthesisUtterance(t);
            window.speechSynthesis.speak(pierreWords);
          } else if(r.result.action == "shopping.find"){
            const shoppingListItems = Shopping.find({user:Meteor.userId()}).fetch();
            console.log("shopping list array");
            console.log(shoppingListItems);
            for(var i = 0; i<shoppingListItems.length; i++){
              var t = shoppingListItems[i].text;
              var pierreWords = new SpeechSynthesisUtterance(t);
              window.speechSynthesis.speak(pierreWords);
            }
          } else if(r.result.action == "fridge.find"){
            const fridgeListItems = Fridge.find({user:Meteor.userId()}).fetch();
            for(var i = 0; i<fridgeListItems.length; i++){
              var t = fridgeListItems[i].text;
              var pierreWords = new SpeechSynthesisUtterance(t);
              window.speechSynthesis.speak(pierreWords);
            }
          }
