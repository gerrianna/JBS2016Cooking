      ...
      Meteor.call("addShoppingTalk",shopping_obj,item, {returnStubValue: true},
        function(error,result){
          ...
          if(r.result.action == "shoppingList.add"){
            ...
            const t = Ins.find().fetch();
            //const instructions = t[0].steps[0].step;
            //var instructions = "";
            for(var i = 0;i<t[0].steps.length;i++){
              console.log(t[0].steps[i].step);
              instructions = t[0].steps[i].step;
              var pierreWords = new SpeechSynthesisUtterance(instructions);
              window.speechSynthesis.speak(pierreWords);
              instructions = "";
              speechSynthesisInstance.pause();
              if(r.result.action = "step.next"){
                
                console.dir("next step resume");
                
                var t = r.result.fulfillment.speech;
                var pierreWords = new SpeechSynthesisUtterance(t);
                window.speechSynthesis.speak(pierreWords);
                var words = new SpeechSynthesisUtterance("hello");
